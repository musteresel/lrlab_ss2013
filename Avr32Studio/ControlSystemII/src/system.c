...
avr32_pwm_channel_t pwm1_channel = { .ccnt = 0 };
LR_pwm pwm1;
avr32_pwm_channel_t pwm2_channel = { .ccnt = 2 };
LR_pwm pwm2;
...
volatile double max_Volt = 8;
volatile double ref_Volt = 0;
...
void con_handler(void)
{
  ...
  double duty_ref = ref_Volt / max_Volt;
  double duty_1 = duty_ref + U;
  double duty_2 = duty_ref - U;
  ...
  if (duty_2 < 0)
  {
    gpio_set_gpio_pin(AVR32_PIN_PX08);
    gpio_clr_gpio_pin(AVR32_PIN_PX09);
    duty_2 *= -1;
  }
  else
  {
    gpio_set_gpio_pin(AVR32_PIN_PX09);
    gpio_clr_gpio_pin(AVR32_PIN_PX08);
  }
  ...
  LR_pwm_update_duty(&pwm1, duty_1/100.);
  LR_pwm_update_duty(&pwm2, duty_2/100.);
  //----------------------------------------------------------------------------
  LR_timer_sti(1);
}
      ...
      if (state.move == SYS_IN_STATE)
      {
        ...
        if (s == SYS_STATE_SET)
        {
          ...
          if (parm == 'F') // PWM FREQ
          {
            ...
            LR_pwm_stop(&pwm1);
            LR_pwm_stop(&pwm2);
            LR_pwm_start(&pwm1, pwm_freq, 0, FOSC0);
            LR_pwm_start(&pwm2, pwm_freq, 0, FOSC0);
            LR_timer_start(1);
          }
          else if (parm == 'P')
          {
...
int main ( void )
{
  ...
  /** Init pwm */
  pwm1.chan = &(pwm1_channel);
  pwm2.chan = &(pwm2_channel);
  gpio_enable_module_pin(AVR32_PIN_PB19, 0); // PWMa out
  gpio_enable_module_pin(AVR32_PIN_PB21, 0); // PWM b out
  gpio_enable_module_pin(AVR32_PIN_PC04, 0); // AIN1
  gpio_enable_module_pin(AVR32_PIN_PC05, 0); // AIN2
  gpio_enable_module_pin(AVR32_PIN_PX08, 3); // BIN1
  gpio_enable_module_pin(AVR32_PIN_PX09, 3); // BIN2
  gpio_clr_gpio_pin(AVR32_PIN_PC04);
  gpio_clr_gpio_pin(AVR32_PIN_PC05);
  gpio_clr_gpio_pin(AVR32_PIN_PX08);
  gpio_clr_gpio_pin(AVR32_PIN_PX09);
